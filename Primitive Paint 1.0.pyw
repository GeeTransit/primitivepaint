...
##Revision Date: Nov 11, 2018                                           ##
##Program Name: Primitive Paint 1.0                                     ##
...
def canvasShape(*args, **kwargs):
    ...
    if (kargs['shape'] == 'line'):
        ...
        if ((kargs['shape'] == 'circle') and (kargs['size'] != 1)):
            return canvas.create_oval(
                x1 - top, y1 - top, x1 + bottom, y1 + bottom,
                fill = kargs['fill'],
                outline = kargs['outline']
            )
        elif ((kargs['shape'] == 'square') and (kargs['size'] != 1)):
...
def undoSingle(*args):
    ...
    if (len(actions) > 1):
        ...
        redo[-1].append(
            ...
            if ((canvas.type(actions[-1][-1])== 'oval'))
...
def redoLast(*args):
    ...
    if (len(redo) > 1):
        ...
        if redo[-1][0]:
            ...
            #loop through every item, adding every third
...
def saveAny(saveAs, *args):
    ...
    for action in actions[1:]:
        ...
        for item in action:
            ...
            file.write(canvas.itemcget(item, 'width') + '\n')
        #end for
    #end for
